...
int		process_xpm(unsigned int flag, char *file, t_base *base)
{
	t_xpm	*ptr;
	...
	if (!(ptr->img = (t_img *)ft_calloc(1, sizeof(t_img))))
		return (1);
	if (!(ptr->img->img = mlx_xpm_file_to_image(base->mlx, file, &ptr->width, \
	&ptr->hight)))
		return (1);
...
int		process_color_fc(unsigned int flag, char *str, t_base *base)
{
	char	**list;
	int		tmp;
	int		result;
